...
import { UofxFilePluginService, UofxFileUploadComponent } from '@uofx/web-components/file';
...
export class AdvancedFieldWriteComponent extends BpmFwWriteComponent implements OnInit {
  @ViewChild('myFileUpload') myFileUpload: UofxFileUploadComponent;
  /** 屬性資料 */
  ...
  setFormValue() {
    if (this.value) {
      ...
      setTimeout(() => {
        if (this.value.attachments && this.value.attachments.files?.length > 0) {
          this.myFileUpload.loadFileList(this.value.attachments.id);
        }
      });
